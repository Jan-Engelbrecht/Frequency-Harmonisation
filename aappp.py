...
def downscale(df, freq, method):
    ...
    if method == "First":
        return df2.resample(pandas_freq).first()
    elif method == "Mean":
        return df2.resample(pandas_freq).mean()
    elif method == "Median":
        return df2.resample(pandas_freq).median()
    else:
        return df2.resample(pandas_freq).mean()
...
def upscale(df, freq, method):
    ...
    if method == "Forward Fill":
        ...
        return df2.interpolate(method="linear", limit_direction="both")
    elif method == "Spline":
        try:
            return df2.interpolate(method="spline", order=2)
        except:
            return df2.interpolate(method="linear", limit_direction="both")
    else:
        return df2.ffill()
...
if uploaded_file:
    ...
    if st.session_state.run_clicked:
        ...
        for name in selected_series:
            ...
            # Replace the date column safely
            df = df.iloc[:, 1:]
            df.index = date_col[valid_mask].values
            ...
            try:
                if name == target_name:
                    ...
                    results[name] = downscale(sheets[name], target_freq, down_method)
 
                elif freq_rank[freq] > freq_rank[target_freq]:
                    results[name] = upscale(sheets[name], target_freq, up_method)
            ...
            for name in selected_series:
                try:
                    ...
                    for col in data.columns:
                        ax.plot(data.index, data[col], label=f"{name}_{col}")
                except:
                    continue
